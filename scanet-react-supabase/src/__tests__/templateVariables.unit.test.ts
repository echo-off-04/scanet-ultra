...
describe('replaceTemplateVariables', () => {
  ...
  const senderProfile = { full_name: 'Pierre Martin', company: 'MonEntreprise' };

  it('replaces {{prenom}} with first name extracted from full name', () => {
    const result = replaceTemplateVariables('Bonjour {{prenom}}', fullContext, null);
    expect(result).toBe('Bonjour Jean');
  });

  it('replaces {{nom_complet}} with full name', () => {
    const result = replaceTemplateVariables('Contact: {{nom_complet}}', fullContext, null);
    expect(result).toBe('Contact: Jean Dupont');
  });

  it('replaces {{entreprise}} with company name', () => {
    const result = replaceTemplateVariables('Chez {{entreprise}}', fullContext, null);
    expect(result).toBe('Chez Acme Corp');
  });

  it('replaces {{evenement}} with event name', () => {
    const result = replaceTemplateVariables('Lors de {{evenement}}', fullContext, null);
    expect(result).toBe('Lors de Tech Summit 2026');
  });

  it('replaces {{date_rencontre}} with event date', () => {
    const result = replaceTemplateVariables('Le {{date_rencontre}}', fullContext, null);
    expect(result).toBe('Le 15/02/2026');
  });

  it('replaces {{source}} with source', () => {
    const result = replaceTemplateVariables('Via {{source}}', fullContext, null);
    expect(result).toBe('Via event');
  });

  it('replaces {{mon_nom}} with sender profile name', () => {
    const result = replaceTemplateVariables('De la part de {{mon_nom}}', fullContext, senderProfile);
    expect(result).toBe('De la part de Pierre Martin');
  });

  it('replaces all variables in a complete email template', () => {
    const template = 'Ravi de vous avoir rencontre, {{prenom}}\n\nBonjour {{prenom}},\n\nSuite a notre rencontre lors de {{evenement}} le {{date_rencontre}}, je souhaitais vous recontacter.\n\nCordialement,\n{{mon_nom}}';
    const result = replaceTemplateVariables(template, fullContext, senderProfile);
  ...
  it('replaces multiple occurrences of the same variable', () => {
    const result = replaceTemplateVariables('{{prenom}} et encore {{prenom}}', fullContext, null);
    expect(result).toBe('Jean et encore Jean');
  });

  it('handles empty context gracefully (all variables become empty)', () => {
    const result = replaceTemplateVariables('Bonjour {{prenom}}, de {{entreprise}}', {}, null);
    expect(result).toBe('Bonjour , de ');
  });

  it('handles missing sender profile gracefully', () => {
    const result = replaceTemplateVariables('Signe: {{mon_nom}}', fullContext, null);
  ...
  it('extracts first name correctly from multi-word name', () => {
    const context = { ...fullContext, contact_name: 'Jean-Pierre De La Fontaine' };
    const result = replaceTemplateVariables('{{prenom}}', context, null);
  ...
  it('handles empty contact_name', () => {
    ...
    const result = replaceTemplateVariables('Bonjour {{prenom}}', context, null);
    expect(result).toBe('Bonjour ');
  });

  it('does not replace non-template text', () => {
    const result = replaceTemplateVariables('Simple text with no variables', fullContext, senderProfile);
    expect(result).toBe('Simple text with no variables');
  });

  it('handles unknown template-like patterns without replacement', () => {
    const result = replaceTemplateVariables('{{unknown}} stays', fullContext, null);
...
describe('buildWhatsAppLink (logic replica)', () => {
  ...
  it('replaces template variables in message', () => {
    const link = buildWhatsAppLink('+33600000000', 'Bonjour {{prenom}} de {{entreprise}}', context);
  ...
  it('encodes message content for URL', () => {
    const link = buildWhatsAppLink('+33600000000', 'Bonjour! Comment allez-vous?', context);
