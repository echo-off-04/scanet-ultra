...
import {
  Home, Users, Calendar, Target, Package, Building2, Settings,
  Bell, LogOut, User, Plus, X, CheckSquare, MoreHorizontal
} from 'lucide-react';
...
export function MobileBottomNav({
  ...
  const handleMoreItemClick = (itemId: ViewType) => {
    onViewChange(itemId);
    ...
    <>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
        <div className="relative flex items-end justify-between px-4 pt-2 pb-3 safe-area-bottom">
          {/* Bouton central surélevé - positionné avant les autres items */}
          <button
            onClick={onAddContact}
            className="absolute left-1/2 -translate-x-1/2 -top-7 w-14 h-14 bg-gradient-to-br from-[#0E3A5D] to-[#1e5a8e] rounded-full shadow-xl flex items-center justify-center hover:scale-105 transition-transform active:scale-95 border-4 border-white z-10"
          ...
          {mainNavItems.map((item, index) => {
            ...
            return (
              <button
                ...
                className={`relative flex flex-col items-center justify-center flex-1 max-w-[70px] py-3 transition-all ${
                  isSecondItem ? 'mr-8' : ''
                } ${
                  isThirdItem ? 'ml-8' : ''
                } ${isActive ? 'text-[#0E3A5D]' : 'text-gray-500'}`}
              >
                ...
                {item.badge !== undefined && item.badge > 0 && (
                  <span className="absolute top-0 right-2 w-4 h-4 bg-red-500 text-white text-[9px] rounded-full flex items-center justify-center font-semibold">
            ...
            <div className="p-4 space-y-2">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-4 rounded-xl transition-colors ${
                      isActive ? 'bg-[#0E3A5D] text-white' : 'hover:bg-gray-50'
                    }`}
                  >
                    <div className="flex items-center gap-3">
              ...
              <div className="p-4 bg-gray-50 rounded-xl">
                <div className="flex items-center gap-3 mb-3">
                  <div className="w-12 h-12 rounded-full bg-gradient-to-br from-[#0E3A5D] to-[#1e5a8e] flex items-center justify-center text-white font-semibold">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 p-3 bg-white rounded-lg hover:bg-gray-100 transition-colors mb-2"
                >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 p-3 bg-white rounded-lg hover:bg-red-50 transition-colors text-red-600"
                >
                ...
                <div className="flex items-center gap-2">
                  {unreadCount > 0 && (
                    <button
                      ...
                      className="text-xs text-blue-600 hover:text-blue-700 font-medium px-3 py-1.5 rounded-lg hover:bg-blue-50"
                    >
                      Tout marquer
                    </button>
                  )}
