...
export function PersonalObjectives({ compact = false }: PersonalObjectivesProps) {
  ...
  const loadObjectives = useCallback(async () => {
    ...
    try {
      ...
      setObjectives((data || []) as PersonalObjective[]);
    } catch (error) {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      const updated = await refreshAllObjectives(user.id);
      ...
      for (const obj of updated) {
        if (obj.status === 'achieved' && !obj.notified) {
          await createNotification({
            ...
            message: `Felicitations ! Vous avez atteint votre objectif "${obj.title}"`,
            priority: 'high',
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'contact_opportunities', filter: `user_id=eq.${user.id}` }, () => {
        debouncedRefresh();
      })
  ...
  const handleReactivate = async (id: string) => {
    ...
    const currentValue = await calculateObjectiveValue(objective);
    const { error } = await supabase
      .from('personal_objectives')
      .update({
        status: 'active',
  ...
  const totalProgress = activeCount > 0
    ? Math.round(
        objectives
          ...
          .reduce((sum, o) => sum + Math.min(100, (o.current_value / (o.target_value || 1)) * 100), 0) / activeCount
      )
    : 0;
  ...
  return (
    <>
      <div className="glass-card p-5 lg:p-6 relative overflow-hidden">
        ...
        <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-5">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#0E3A5D] to-[#1e5a8e] flex items-center justify-center shadow-md">
              ...
              <div className="flex items-center gap-3 text-xs text-gray-500">
                ...
                {achievedCount > 0 && (
                  <>
                    ...
                    <span className="text-emerald-600 font-medium">{achievedCount} atteint{achievedCount > 1 ? 's' : ''}</span>
                  </>
                )}
                {activeCount > 0 && (
                  <>
                    <span className="w-1 h-1 rounded-full bg-gray-300" />
                    <span className="font-medium">{totalProgress}% global</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {!compact && (
              <div className="relative">
                ...
                >
                  ...
                  {filterStatus === 'all' ? 'Tous' : filterStatus === 'active' ? 'Actifs' : 'Atteints'}
                ...
                {showFilters && (
                  <div className="absolute right-0 top-full mt-1 w-36 bg-white rounded-xl shadow-xl border border-gray-200 z-20 overflow-hidden">
                    {[
                      ...
                      <button
                        key={f.value}
                        onClick={() => { setFilterStatus(f.value as 'all' | 'active' | 'achieved'); setShowFilters(false); }}
                        className={`w-full text-left px-3 py-2 text-sm transition-colors ${
                          filterStatus === f.value ? 'bg-blue-50 text-blue-700 font-medium' : 'text-gray-600 hover:bg-gray-50'
                        }`}
                      >
                        {f.label}
                      </button>
            ...
            <button
              onClick={() => { setEditObjective(null); setShowAddModal(true); }}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#0E3A5D] text-white rounded-xl text-xs font-semibold hover:bg-[#1e5a8e] transition-colors shadow-md shadow-blue-900/10"
            >
        ...
        {filteredObjectives.length === 0 ? (
          <div className="text-center py-10">
            ...
            <h4 className="font-semibold text-gray-700 mb-1">
              {filterStatus !== 'all' ? 'Aucun objectif dans cette categorie' : 'Aucun objectif defini'}
            </h4>
            <p className="text-sm text-gray-500 mb-4 max-w-sm mx-auto">
              {filterStatus !== 'all'
                ? 'Changez le filtre pour voir d\'autres objectifs'
                : 'Fixez-vous des objectifs mesurables pour suivre votre progression en temps reel'
              }
            </p>
            {filterStatus === 'all' && (
              <button
                onClick={() => { setEditObjective(null); setShowAddModal(true); }}
                className="inline-flex items-center gap-2 px-4 py-2 bg-[#0E3A5D] text-white rounded-xl text-sm font-semibold hover:bg-[#1e5a8e] transition-colors"
              >
                <Plus className="w-4 h-4" />
                Creer mon premier objectif
              </button>
            )}
          </div>
        ) : (
          <div className={`grid gap-3 ${compact ? 'grid-cols-1 sm:grid-cols-2' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>
            {filteredObjectives.map((objective) => (
              <ObjectiveProgressCard
                key={objective.id}
                objective={objective}
                onEdit={(obj) => { setEditObjective(obj); setShowAddModal(true); }}
      ...
      {showAddModal && (
        <AddObjectiveModal
          onClose={() => { setShowAddModal(false); setEditObjective(null); }}
          onSuccess={() => {
