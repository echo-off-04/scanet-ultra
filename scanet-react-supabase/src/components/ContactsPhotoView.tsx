...
export function ContactsPhotoView({ contacts, onContactClick }: ContactsPhotoViewProps) {
  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      ...
      prospect: 'from-blue-500 to-blue-700',
      client: 'from-emerald-500 to-emerald-700',
      partner: 'from-purple-500 to-purple-700',
      collaborateur: 'from-cyan-500 to-cyan-700',
      ami: 'from-pink-500 to-pink-700',
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5 sm:gap-6 lg:gap-8">
      {contacts.map((contact) => (
        <button
          ...
          className="group flex flex-col items-center text-center transition-all hover:scale-110 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 rounded-2xl p-2"
        >
          <div className="relative mb-3">
            ...
            {contact.rating && contact.rating > 0 && (
              <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-sm rounded-full px-2.5 py-1 shadow-lg flex items-center gap-0.5 border border-white/60">
                ...
                <span className="text-xs font-bold text-gray-700">{contact.rating}</span>
              </div>
            )}
          </div>
          <div className="w-full px-1">
            <h3 className="font-semibold text-gray-900 text-xs sm:text-sm mb-1 line-clamp-2 group-hover:text-primary transition-colors">
              {contact.full_name}
            </h3>
            {contact.job_title && (
              <p className="text-xs text-gray-500 mb-1 line-clamp-1 hidden sm:block">{contact.job_title}</p>
            )}
            {contact.company && (
              <div className="flex items-center justify-center gap-1 text-xs text-gray-600 mb-2 hidden sm:flex">
