import { Mail, Phone, Building2, MapPin, Star, Tag } from 'lucide-react';
            ...
            <tbody className="divide-y divide-gray-100">
              {contacts.map((contact) => (
                <tr
                  key={contact.id}
                  onClick={() => onContactClick(contact)}
                  className="hover:bg-white/60 cursor-pointer transition-all"
                >
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(contact.status || 'lead')}`}>
                      {getStatusLabel(contact.status || 'lead')}
                    </span>
                  </td>
                  <td className="px-6 py-4">
