...
export function EventsList({ onEventClick, onCreateEvent }: EventsListProps) {
  ...
  const filterAndSortEvents = () => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'participants':
          return b.actual_participants - a.actual_participants;
        case 'performance':
 ...
 const totalContacts = events.reduce((sum, e) => sum + e.contacts_added, 0);
  ...
  const avgScore = events.length > 0 
    ? events.reduce((sum, e) => sum + e.performance_score, 0) / events.length 
    : 0;
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div>
          <p className="text-lg text-gray-500">{events.length} événement{events.length !== 1 ? 's' : ''}</p>
        </div>
        ...
        >
          ...
          <span className="relative z-10">Créer un événement</span>
        </button>
      </div>

  {/* Stats en grille moderne - Style Dashboard */}
{events.length > 0 && (
  <div className="grid grid-cols-2 gap-3 lg:gap-4">
    {/* Total Events - Dark card */}
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2 lg:mb-3">
            <div className="p-1.5 rounded-lg transition-transform duration-300 group-hover:scale-110 bg-white/10">
              <Calendar className="w-3.5 h-3.5 transition-colors duration-300 text-white/90" strokeWidth={2} />
            </div>
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2 lg:mb-3">
            <div className="p-1.5 rounded-lg transition-transform duration-300 group-hover:scale-110" style={{ backgroundColor: '#3b82f615' }}>
              <Target className="w-3.5 h-3.5 transition-colors duration-300" strokeWidth={2} style={{ color: '#3b82f6' }} />
            </div>
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2 lg:mb-3">
            <div className="p-1.5 rounded-lg transition-transform duration-300 group-hover:scale-110" style={{ backgroundColor: '#10b98115' }}>
              <UserPlus className="w-3.5 h-3.5 transition-colors duration-300" strokeWidth={2} style={{ color: '#10b981' }} />
            </div>
            <p className="text-[10px] lg:text-xs font-medium uppercase tracking-wider text-gray-500">
              Contacts
            </p>
    ...
    <div className="group relative overflow-hidden">
      <div className="relative rounded-xl lg:rounded-2xl p-4 lg:p-5 border transition-all duration-300 hover:-translate-y-0.5 overflow-hidden bg-white border-gray-100 shadow-[0_2px_8px_rgba(0,0,0,0.08)] hover:shadow-[0_4px_16px_rgba(0,0,0,0.12)]">
        ...
        {/* Light card effects */}
        <div className="absolute inset-0 bg-gradient-to-br from-purple-500/10 to-purple-600/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-xl lg:rounded-2xl" />
        <div className="absolute inset-0 bg-gradient-to-br from-white/40 via-white/0 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-xl lg:rounded-2xl" />
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-2 mb-2 lg:mb-3">
            <div className="p-1.5 rounded-lg transition-transform duration-300 group-hover:scale-110" style={{ backgroundColor: '#8b5cf615' }}>
              <TrendingUp className="w-3.5 h-3.5 transition-colors duration-300" strokeWidth={2} style={{ color: '#8b5cf6' }} />
            </div>
            <p className="text-[10px] lg:text-xs font-medium uppercase tracking-wider text-gray-500">
              Score
            </p>
      ...
      <div className="bg-white rounded-3xl border border-gray-200 p-6">
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <button
            ...
            className={`px-8 py-4 rounded-2xl transition-all font-semibold whitespace-nowrap flex items-center gap-3 ${
          ...
          <div className="mt-6 pt-6 border-t border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-3 bg-gray-50 border-0 rounded-2xl focus:outline-none focus:ring-2 focus:ring-gray-900 transition-all text-gray-900"
                >
            ...
            {hasActiveFilters && (
              <button
                ...
                className="mt-6 flex items-center gap-2 text-sm text-gray-900 hover:text-gray-700 font-semibold transition-colors"
              >
                <X className="w-4 h-4" />
                Réinitialiser
              </button>
      ...
      {filteredEvents.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEvents.map((event) => {
            ...
            const progressPercentage = event.target_participants > 0
              ? Math.min(100, (event.actual_participants / event.target_participants) * 100)
              : 0;
            ...
            return (
              <div
                ...
                className="group relative bg-white rounded-3xl border border-gray-200 overflow-hidden hover:border-gray-900 hover:shadow-2xl transition-all duration-300 cursor-pointer"
              >
                {/* Header avec image ou gradient noir */}
                <div className="relative h-48 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 overflow-hidden">
                  {event.image_url ? (
                    ...
                    <>
                      ...
                      <div className="absolute inset-0 opacity-10">
                        <svg className="w-full h-full" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
                          <defs>
                            <pattern id={`dots-${event.id}`} x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
                              <circle cx="2" cy="2" r="1" fill="white" />
                            </pattern>
                          </defs>
                          <rect width="100%" height="100%" fill={`url(#dots-${event.id})`} />
                        </svg>
                      </div>
                  ...
                  <div className="absolute top-4 right-4">
                    <span className={`px-4 py-2 rounded-full text-xs font-bold text-white bg-gradient-to-r ${statusInfo.gradient} shadow-lg backdrop-blur-sm`}>
                      {statusInfo.label}
                    </span>
                  </div>
                  ...
                  {(!event.target_participants || event.target_participants === 0) && (
                    <div className="absolute bottom-4 left-4">
                      <span className="px-3 py-1.5 rounded-full text-xs font-bold bg-amber-400 text-gray-900 flex items-center gap-1.5 shadow-lg">
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-3 mb-5">
                    {event.location && (
                      <div className="flex items-center gap-3 text-sm text-gray-700">
                        <TypeIcon className="w-4 h-4 text-gray-400" strokeWidth={2} />
                  ...
                  <div className="mb-5">
                    <div className="flex items-center justify-between text-sm mb-2">
                      ...
                      <span className="font-bold text-gray-900">
                        {event.actual_participants} / {event.target_participants || 0}
                      </span>
                    </div>
                    <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
                      <div
                        className="h-full rounded-full bg-gradient-to-r from-gray-900 to-gray-700 transition-all duration-500"
                  ...
                  <div className="pt-5 border-t border-gray-100">
                    <div className="grid grid-cols-3 gap-4">
                      <div> 
                        <p className="text-2xl font-bold text-gray-900">{event.leads_generated}</p>
                        <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mt-1">Leads</p>
                      </div>
                      <div>
                        <p className="text-2xl font-bold text-gray-900">{event.contacts_added}</p>
                        <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mt-1">Contacts</p>
                      </div>
                      <div>
                        <div className="flex items-center gap-1">
                          <p className="text-2xl font-bold text-gray-900">{event.performance_score.toFixed(0)}</p>
                          <TrendingUp className="w-4 h-4 text-gray-400" />
                        </div>
                        <p className="text-xs text-gray-500 font-semibold uppercase tracking-wider mt-1">Score</p>
                      </div>
                    </div>
                  </div>
