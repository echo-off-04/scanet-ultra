...
import {
  Users, Calendar, CheckSquare, Filter, TrendingUp, LogOut,
  ChevronLeft, ChevronRight, Target, BarChart3, Settings,
  Home, ChevronDown, ChevronUp, User, Package, Building2, Bell, X, Check, CheckCheck
} from 'lucide-react';
import { useNotifications } from '../contexts/NotificationContext';

export type ViewType = 'dashboard' | 'contacts' | 'events' | 'followups' | 'opportunities' | 'offers' | 'enterprise' | 'settings';
...
export function Sidebar({
  ...
  const filterOptions = [
    ...
    { value: 'collaborateur', label: 'Collaborateurs', count: (stats as any).collaborateurs || 0, color: 'bg-cyan-100 text-cyan-700' },
    { value: 'ami', label: 'Ami(e)s', count: (stats as any).amis || 0, color: 'bg-pink-100 text-pink-700' },
    { value: 'fournisseur', label: 'Fournisseurs', count: (stats as any).fournisseurs || 0, color: 'bg-amber-100 text-amber-700' },
  ];
  ...
  return (
    <>
      <aside
        className={`hidden lg:flex bg-white border-r border-gray-200 flex-col h-screen transition-all duration-300 ease-in-out ${
      ...
      <div className={`p-4 border-b border-gray-200 ${isCollapsed ? 'px-2' : 'p-6'}`}>
        ...
        {isCollapsed && (
          <button
            onClick={toggleCollapsed}
            className="hidden lg:flex w-full mt-3 p-2 hover:bg-gray-100 rounded-xl transition-colors justify-center"
      ...
      <div className={`relative border-b border-gray-200 ${isCollapsed ? 'p-2' : 'p-3'}`}>
        <button
          ...
          className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} px-3 py-2.5 rounded-xl font-medium transition-all text-gray-700 hover:bg-gray-100 relative`}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            ...
                            <span className={`text-xs px-2 py-0.5 rounded-full ${
                              notification.category === 'opportunities' ? 'bg-green-100 text-green-700' :
                              notification.category === 'contacts' ? 'bg-blue-100 text-blue-700' :
                              notification.category === 'follow_ups' ? 'bg-orange-100 text-orange-700' :
                              notification.category === 'events' ? 'bg-purple-100 text-purple-700' :
      ...
      <nav className="flex-1 overflow-y-auto p-2 space-y-1">
            ...
            return (
              <div key={item.id} className="relative group">
                <button
                  ...
                  className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} px-3 py-2.5 rounded-xl font-medium transition-all ${isActive
                    ? 'bg-[#0E3A5D] text-white shadow-md'
                ...
                >
                  ...
                  {isCollapsed && item.count !== null && item.count > 0 && item.badge && (
                ...
                {isCollapsed && (
                  <div className="absolute left-full top-1/2 -translate-y-1/2 ml-2 px-3 py-1.5 bg-gray-900 text-white text-sm rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all whitespace-nowrap z-50">
                    {item.label}
        ...
        {/* Filtres (seulement sur vue contacts et si pas collapsed) */}
                ...
                {filterOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-semibold ${isActive ? 'bg-blue-100 text-blue-700' : option.color
                          }`}
                      >
                        {option.count}
                      </span>
        ...
        {bottomMenuItems.map((item) => {
          ...
          return (
            <div key={item.id} className="relative group p-2">
              <button
                onClick={() => onViewChange(item.id)}
                className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} px-3 py-2.5 rounded-xl font-medium transition-all ${isActive
        ...
        <div className={`p-3 ${isCollapsed ? 'px-2' : ''}`}>
          <div className="relative">
            <button
              ...
              className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'} p-2 rounded-xl hover:bg-gray-50 transition-all`}
            >
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#0E3A5D] to-[#1e5a8e] flex items-center justify-center text-white font-semibold text-sm flex-shrink-0">
          ...
          {/* Bouton déconnexion pour mode collapsed */}
          {isCollapsed && (
            <div className="relative group mt-2">
              <button
                onClick={onSignOut}
                className="w-full flex justify-center p-2.5 text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors"
                title="Déconnexion"
              >
