...
export function AddObjectiveModal({ onClose, onSuccess, editObjective }: AddObjectiveModalProps) {
  ...
  const [objectiveType, setObjectiveType] = useState<string>(editObjective?.objective_type || '');
  const [title, setTitle] = useState(editObjective?.title || '');
  const [description, setDescription] = useState(editObjective?.description || '');
  const [targetValue, setTargetValue] = useState(editObjective?.target_value?.toString() || '');
  const [currency, setCurrency] = useState(editObjective?.currency || userCurrency);
  const [contactStatusFilter, setContactStatusFilter] = useState(editObjective?.contact_status_filter || 'lead');
  const [periodType, setPeriodType] = useState(editObjective?.period_type || 'month');
  const [priority, setPriority] = useState(editObjective?.priority || 'medium');
  const [eventId, setEventId] = useState(editObjective?.event_id || '');
  ...
  const initDates = editObjective?.period_start && editObjective?.period_end
    ? { start: editObjective.period_start.split('T')[0], end: editObjective.period_end.split('T')[0] }
    : getDefaultDates(editObjective?.period_type || 'month');

  const [startDate, setStartDate] = useState(initDates.start);
  const [endDate, setEndDate] = useState(initDates.end);

  useEffect(() => {
    if (objectiveType === 'participation_rate') {
      loadEvents();
    }
  }, [objectiveType]);
  ...
  useEffect(() => {
    if (objectiveType && !editObjective) {
      ...
      if (config) {
        if (objectiveType === 'contacts_by_status') {
          setTitle(`${CONTACT_STATUS_LABELS[contactStatusFilter]} ${PERIOD_LABELS[periodType] || 'personnalise'}`);
        } else if (objectiveType === 'new_contacts') {
          setTitle(`Nouveaux contacts ${PERIOD_LABELS[periodType] || 'personnalise'}`);
        } else {
  ...
  const handleSave = async () => {
    if (!user || !objectiveType || !targetValue || !title) return;
  ...
  const selectedConfig = objectiveType
    ? OBJECTIVE_TYPE_CONFIG[objectiveType as keyof typeof OBJECTIVE_TYPE_CONFIG]
    : null;
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">
                {editObjective ? 'Modifier l\'objectif' : 'Nouvel objectif'}
              </h2>
              <p className="text-xs text-gray-500">Definissez un objectif mesurable</p>
            </div>
          </div>
          ...
          {objectiveType && (
            <>
              {objectiveType === 'contacts_by_status' && (
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Statut a suivre</label>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Atteindre 50 000 EUR de CA"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    Objectif cible
                    {selectedConfig?.unit === 'percentage' && ' (%)'}
                    {selectedConfig?.unit === 'currency' && ` (${currency})`}
                  </label>
                  <input
                    type="number"
                    value={targetValue}
                    ...
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400"
                    placeholder={selectedConfig?.unit === 'percentage' ? '80' : '100'}
                    min="0"
                    step={selectedConfig?.unit === 'currency' ? '100' : '1'}
                  />
                </div>
              ...
              {objectiveType === 'participation_rate' && (
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Evenement specifique (optionnel)</label>
                  <div className="relative">
                    <select
                      value={eventId}
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-100 p-5 flex gap-3 rounded-b-2xl">
          ...
          <button
            onClick={handleSave}
            disabled={!objectiveType || !targetValue || !title || saving}
          ...
          >
            {saving ? 'Enregistrement...' : editObjective ? 'Modifier' : 'Creer l\'objectif'}
          </button>
        </div>
      </div>
    </div>
